...
class MyTheme_Contact_Info_Widget extends WP_Widget {
	public function __construct() {
		$widget_ops = array(
			...
			'description' => esc_html__( 'Hiển thị thông tin liên hệ', 'themeblank' ),
		);
		...
		<div class="list">
			...
			<?php if ( ! empty( $instance['phone'] ) ) : ?>
				<div class="item">
					...
					<a class="text"
					   href="tel:<?php echo esc_attr( themeblank_preg_replace_ony_number( $instance['phone'] ) ); ?>"><?php echo esc_html( $instance['phone'] ); ?></a>
				</div>
			<?php endif; ?>
		</div>
		<?php
	...
	public function form( $instance ): void {
		$defaults = [
			'title'   => esc_html__( 'Thông tin liên hệ', 'themeblank' ),
			'address' => '',
			'email'   => '',
			'phone'   => ''
		];
		...
		$fields = [
			'title'   => esc_html__( 'Tiêu đề:', 'themeblank' ),
			'address' => esc_html__( 'Địa chỉ:', 'themeblank' ),
			'email'   => esc_html__( 'Email:', 'themeblank' ),
			'phone'   => esc_html__( 'Số điện thoại:', 'themeblank' )
		];
...
function themeblank_register_contact_info_widget(): void {
	register_widget( 'MyTheme_Contact_Info_Widget' );
}

add_action( 'widgets_init', 'themeblank_register_contact_info_widget' );