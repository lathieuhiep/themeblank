...
$show_loading = themeblank_opt(GeneralOptions::class)::get_loading_enabled() ?? true;

if(  $show_loading ) :
    $opt_image_loading  = themeblank_opt(GeneralOptions::class)::get_loading_image_id();
?>
    <div id="site-loading" class="d-flex align-items-center justify-content-center">
        <?php if ( !empty( $opt_image_loading ) ): ?>
            <img class="loading_img"
                 ...
                 alt="<?php esc_attr_e('Đang tải...','themeblank') ?>">
        <?php else: ?>
            <img class="loading_img"
